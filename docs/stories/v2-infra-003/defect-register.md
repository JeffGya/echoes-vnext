...
| **D57** | Movement adapter criterion 5 orders cells by lexicographic `"col,row"` | **Fixed** | V2-COMBAT-003 phase 10. Criterion 5 is now two numeric criteria, `col` then `row`, in `StagePartyMovementAdapter.select_objective_target`. The order is monotone in each axis and no longer changes its favoured compass direction with coordinate magnitude. **The premise held; the blast radius did not.** A probe over the three shipped realms (15 generated stages, boards 30 to 38 cells wide) found the two orders differ on **10 of 15 boards** — for example realm.prologue stage 0, where the string order is 12,13 / 16,7 / 23,13 / 4,8 and the numeric order is 4,8 / 12,13 / 16,7 / 23,13. So the defect is real on ordinary coordinates. **But nothing moved, and nothing could.** Criterion 5 is reached only after weight, distance AND id all tie, and `RealmGenerator` gives every situation a unique `sit.<n>` id: the same probe counted **0 duplicate ids and 0 id-less situations** across all 15 boards, so criterion 4 always decides first. The guard is correct for a future producer that emits id-less or duplicate entries; it decides nothing today. Full suite unchanged, no fingerprint moved. **The in-file docblock prescribed frontier criterion 4's salted `_fnv1a_32` instead; that prescription is withdrawn** — a de-aligner exists to remove systematic compass drift over a ring of equidistant candidates, which this criterion never sees, and it would need a salt threaded through three `ActiveStageService` call sites for no measured gain. New fixture: `objective_tie_break_is_numeric_not_lexicographic`. |
