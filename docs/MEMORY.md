...
| CombatTokenPresentationState (V2-COMBAT-002 S6D) | `ui/screens/combat/CombatTokenPresentationState.gd` | Pure `RefCounted`, pixel-space only — never sees a grid cell, board, or scene tree, so it stays unit-testable. Walks a per-segment waypoint queue instead of a single straight lerp, so tokens step cell-by-cell and cannot cut through walls. Waypoints arrive ALREADY converted by `CombatTokenLayer`/`CombatBoardScreen` through the same `_board.map_to_local` + `_draw_pos` calls that build every token, so a waypoint is pixel-identical by construction. `seg_duration = move_duration / seg_count` ⇒ TOTAL animation time is invariant to path length, which is what keeps the `$AutoTimer` from firing mid-animation (no busy flag exists or is needed). Queue arms only on a real cell transition for the acting mover, so a stale snapshot refresh cannot re-fire it or jump the token back. |
| CombatPressureService (V2-COMBAT-002 S2) | `core/movement/CombatPressureService.gd` | Dormant perceived-fact adapter for COMBAT, PURIFY_SHRINE, RECOVER, PROTECT, ENDURE, PURSUE, and GUIDE_SPIRIT. Produces at most three fully validated direct/tactical/safety goals with stable IDs and sources; never mutates combat or decides objective, custody, progress, or victory truth. **S4 extensions:** board fall-back reuses the existing `withdraw` purpose (no new purpose/style; `RetreatService` untouched); PURSUE escape-graph cutoff is **primary**, overriding the authored `fallback_region`; PROTECT stolen-carrier cutoff projects from the carrier's own escape graph; interposition/screen regions via a Chebyshev-geodesic predicate, bounds + walkability filtered. |
...
| MovementExecutor (V2-COMBAT-002 S3) | `core/movement/MovementExecutor.gd` | Dormant pure edge-by-edge traversal. `execute(context, intent, profile, hazard_ctx)`. Two-solid-corners diagonal via `StageTerrain.is_legal_edge`; dynamic occupancy; once-per-edge hostile-control surcharge (+1, sources sorted); free forced displacement; strictly ordered events. Stop reasons: reached_destination / commitment_spent / capacity_spent / no_route / blocked_edge / occupied / binding_stop. **S4 extension:** authored-override movers get a one-**STEP** pace independent of edge cost — `step_allowance = mini(authored_override.capacity, commitment)` counted in **EDGES**, so a single edge is always affordable regardless of terrain cost or the `+1` hostile-control surcharge. Normal movers keep byte-identical cost-budget semantics; `voluntary_cost` still records true spend; `remaining_capacity` clamps at 0. |
...
| PursueEscapeService (V2-COMBAT-002 S4) | `core/movement/PursueEscapeService.gd` | Dormant **single escape authority** for PURSUE. `escape_cells` / `is_escaped` / `escape_graph` / `cutoff_cells`. The `>= max-1` **band** is the one predicate (`_is_escape_geometry`); `is_escaped` byte-replicates `FlowRuntime.gd:2082-2087` so the Slice-6 cutover is behavior-preserving. `cutoff_cells` projects a shortest-route corridor off the escape graph (`cost_to_quarry + cost_to_escape == min_escape_cost`), filtered to cells a pursuer reaches no later than the quarry. Consolidates four previously inconsistent inline escape computations. |
| GuideSpiritActivationService (V2-COMBAT-002 S4) | `core/movement/GuideSpiritActivationService.gd` | Dormant non-joining GUIDE spirit activation. `activate_spirit(spirit_actor, context, guide_state, hazard_ctx, capacity_cfg)` routes the spirit's objective-phase movement through `CombatActivationService` + hazards. Authored one-cell pace via `authored_override` capacity 1; hard `joined` precondition **refuses** joined spirits. Escort = first step of `shortest_path`; skittish = neighbour ranked Chebyshev → Manhattan → threat-distance vector → obstruction-distance vector → numeric (reflection-covariant). Scores **no** objective progress (forced `0.0`). |
| ProtectCustodyService (V2-COMBAT-002 S4) | `core/movement/ProtectCustodyService.gd` | Dormant PROTECT custody mechanics over an **immutable** `custody_state`. Pickup **IS** the activation's single primary action (`protect.totem_pickup`, `pickup_range` 1); totem follows the carrier across voluntary **and** forced steps; −1 carrier capacity burden respecting the capacity-1-requires-`authored_override` invariant; theft transfers only on a successful **ATTACK** (injected roll, strict `<`, **no internal RNG**) and **never** on cell entry (explicit tested no-op); enemy carrier is move/action-restricted and takes double damage via the existing `double_damage_mult`; drop/recovery cell = carrier's current cell. |
| StagePartyMovementAdapter (V2-COMBAT-002 S5) | `core/movement/StagePartyMovementAdapter.gd` | Dormant STAGE-traversal bridge onto the movement contracts. `build_profile` / `build_goal` / `build_intent` / `build_result` / `select_frontier` / `select_objective_target`. Pure static, no RNG, no `OS` time, zero references to `FlowRuntime`/`FlowContext`/`SaveService`/UI, **zero production callers**. **ONE PARTY TOKEN** (`MOVER_ID = "party.token"`) — no per-Echo seam is exposed, by design. Capacity is the directive `step_budget` (Scout 3 / Seek 6), not the clamp(2,6) combat formula; below 2 it emits an `authored_override` naming `directive.step_budget`. `controlling_state` false. Reachability via `MovementPathService`, so the shared two-solid-corners diagonal rule applies — `StageTerrain.bfs_distance_field` is deliberately NOT used (raw `walkable.has()` ignores that rule). Frontier tie-break carries a **known, pinned bias**: no `heading` is produced anywhere in `core/` today, so the lexicographic `"col,row"` guard is the effective tie-breaker. |
...
**V2-COMBAT-002 Slice 4 acceptance:** three new dormant pure/static movement services — `PursueEscapeService`, `GuideSpiritActivationService`, `ProtectCustodyService` — plus additive extensions to `CombatPressureService` (board fall-back on the existing `withdraw` purpose, PURSUE/PROTECT cutoff projection, interposition regions) and `MovementExecutor` (authored-override one-**STEP** pace). **Escape authority unified:** the `>= max-1` band is the single predicate and `is_escaped` byte-replicates `FlowRuntime.gd:2082-2087`, so the Slice-6 cutover is behavior-preserving; four inconsistent inline escape computations collapse into one seam. **Custody model:** immutable `custody_state`, pickup **is** the single primary action, theft transfers only on a successful attack (injected roll, no internal RNG) and never on cell entry, enemy carrier restricted + double damage via the existing `double_damage_mult`. **Authored pace is a STEP COUNT, not a cost budget** (`docs/movement-model.md` §13.7) — this fixed a confirmed bug where the non-joining spirit could never move with a hostile adjacent or terrain cost ≥ 2, leaving PROTECT inert exactly when it mattered (a regression vs V1's cost-free BFS); the rule keys off the `authored_override` contract, so it also covers burdened PROTECT carriers. **Action economy unchanged** — capacity is movement only, one primary action after movement; the proposed unified action-point pool is **deferred to its own story** (`docs/proposals/action-point-economy.md`, untracked). **Dormant only** — nothing is wired into live combat; the live cutover is Slice 6. Final verification: **1233/1233 full suite** (baseline 1149 → 1233, +84), 0 failed; compile clean; five independent reviews plus a targeted second pass over `GuideSpiritActivationService` and the four new suites, which caught and fixed three genuine defects (malformed `escape_cells` ternary returning the far-end line instead of the band; the authored-pace step-count pin; two un-reverted mutation-test edits). Jeff's manual regression pass confirmed nothing regressed. Slice-6 carry-forward items (NOT Slice-4 defects) are recorded in `docs/integration-map.md`; party stage adapter → Slice 5, presentation + live cutover → Slice 6.

**V2-COMBAT-002 Slice 5 acceptance:** one new dormant pure/static file — `StagePartyMovementAdapter` — plus an allowlist append of `"explore"` to `MovementGoal.MODES`, and **one authorized live correction**. **Adapter:** converts party/directive state into ONE shared profile/goal/intent/result for STAGE traversal, against a single party token; it has zero production callers and goes live at the Slice-6 cutover. Capacity is the directive `step_budget`, not the combat capacity formula; reachability uses `MovementPathService` (shared diagonal rule), deliberately not `StageTerrain.bfs_distance_field`. **LIVE correction — path excludes origin:** `traveled_path` / `last_traveled_path` are now destinations-only, matching `MovementContractValidation.require_path_excludes_origin`; the pre-move cell is carried by the new additive `data.traveled_origin` / `explore_map.last_traveled_origin`, and the UI tween was re-anchored. **Rendered animation, ghost trail and step-diamond count are unchanged**, and the save change is **additive** — `last_traveled_path` is neither removed nor renamed. Live exploration routing (`nearest_unexplored`, `next_step`, `_find_explore_target` tiers 1–2) is untouched. **Known bias, pinned not fixed:** `heading` is not produced anywhere in `core/`, so the frontier tie-break's lexicographic `"col,row"` guard is the effective tie-breaker on symmetric boards; a characterisation test pins the current biased answers and it must be replaced, not repaired, at Slice 6 — this is the **top Slice-6 cutover risk**. Final verification: **1256/1256 full suite** (baseline 1233 → 1256, +23), 0 failed. Slice-6 carry-forward items (NOT Slice-5 defects) are recorded in `docs/integration-map.md` alongside the Slice-3 and Slice-4 lists, which all remain in force. Presentation + live cutover → Slice 6.
