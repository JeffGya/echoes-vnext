...
- V2-COMBAT-002 built the shared movement layer underneath this across dormant slices 1–5 (contracts and weighted paths, pressure/route arbitration, profile derivation plus a shared executor and hazards, the special PURSUE/GUIDE/PROTECT mechanics, and a stage-party adapter that expresses exploration traversal in the same contract vocabulary as combat — one party token, not per-Echo) and, in Slice 6 (6A cutover contract + config prep dormant; **6B+6C now LIVE**), cut it over from dormant to live for combat and stage. Actor movement now resolves through `CombatActivationService` / `MovementExecutor`; the arbiter's movement-aware selection and party-based stage traversal are live; `GridService.move_toward` is retired. The cutover fixed two bugs: actors previously froze one tile short of contact in every mode — the live planner now models the executor's hostile-control surcharge (new seam `MovementOptionService.hostile_edge_costs` threaded into all live planner calls; legacy commitment uses the surcharged route cost); and combat destinations previously converged on the top-left cell — the tie-break now uses the same salted FNV-1a de-aligner as the 6A stage frontier selection (new seam `StagePartyMovementAdapter.salted_cell_hash`, keyed by mover). The four movement rule shapes (combat `GridService`, exploration `StageTerrain`, escape checks, spirit/carrier pacing) converge there. Slices 6D/6E then closed the story: the minimal token-path animation shipped, and activating the selector for the first time exposed and fixed FIVE real defects. Two were P1 and had made the whole cutover INERT in live play — `_movement_build_direct_option` emitted an `option_id` the contract always rejected (so `options` was always empty, `selection_enabled` always false, and `BehaviorArbiter.select_movement_intent` never ran), and `_crosscheck_perceived_actor` compared `controlling_state` against a raw actor field nobody sets, discarding the ENTIRE board whenever a structure, corpse or KO'd actor was present. Also fixed: `duplicate_goal_id` (goal ids carry no target discriminator, so two goals about two different hostiles could collide and cost the mover all three goals), selection gated on options rather than goals (4–13% of activations fell back to legacy), a destroyed objective still published as alive, order-dependent occupancy under stacking, an unguarded capacity divide, and a PURSUE spawn geometry bug where the quarry's row tie-break anchored to the BOARD midpoint rather than the party's spawn band — a Chebyshev gap the depth fraction never governed, which made the mode losable by geometry alone on tall boards. The story also now publishes truthful `hostile_control_sources` and `hazard_summary` (previously hardcoded empty) as the declared handoff to `V2-COMBAT-003`. **Deliberately NOT done here, and owned by `V2-COMBAT-003` (Movement Model Slice C):** the `exposure`/`cohesion`/`congestion` option fields remain `0.0` because they are WEIGHTED terms in `_spatial_utility` — filling them activates scoring weights that have never run — and purify positioning above 50% shrine health, which cannot be published without reweighting arbitration (an attempt flipped 3 of 20 seeded encounters from win to `shrine_destroyed`). The split is deliberate: control/hazard summaries are declarative route FACTS (this story's to publish); risk appetite is COMBAT-003's to decide.
- V2-COMBAT-003 (2026-09-07) is Done and closes the movement handoff above. It gives combat one deterministic behavior-arbitration and explanation authority: a `DecisionTrace` and per-Echo `GuidanceContribution` on `BehaviorArbiter`, so a decision can now say why it was made and prove it, and the Keeper's guidance and each Echo's own read of it are both visible. Separately, this story rewrote terrain: islands are moated by design, bridges are their own tile carrying a `kind` field (connect/island/density), and each realm's board now reads with its own character rather than a shared shape. Bridge rendering took two passes to get right — `z_index` removed first, then `y_sort_enabled` added to `BridgeLayer` once play showed the tint still painting above the board — because a child of a y-sorted parent only joins the per-row draw list when it sets `y_sort_enabled` itself. Two pre-existing defects surfaced and were fixed in passing, not as story scope: once islands stopped being single cells, a mid-round wave spawn (`CombatRoundSpawnService._place_enemy_spawns`) could land on one with no host-region filter; and a fight where an Echo is fear-refused and the enemy only guards had no round cap and could run forever, so a no-progress detector now ends it as a forced retreat that pays nothing. The owner ran three play-test sessions on a fresh save and accepted the result (`docs/stories/v2-combat-003/handoff.md` §26): boards, bridges, and the wave-spawn fix confirmed across both realms; the Absolute Fear Rule confirmed firing at the nascent threshold; the forced-retreat path is proven by test and measurement but not observed in play, because the owner could not force the stalemate that triggers it; guidance/voice barks and log lines confirmed. One defect surfaced in play and was not fixed: the `combat_emotion` debug command throws, because it reads a private field that `CombatTokenLayer.set_emotion_debug()` deliberately no longer sets — filed to V2-COMBAT-003.5, and unrelated to the still-working `emotion` debug command.
- V2-COMBAT-003.5 (2026-09-26) is Done, merged across 7 PRs (#69, #74, #75, #76, #77, #79, #80 — decision #42: one PR per phase, after `/ultrareview` refused an oversized single-PR attempt). Builds the movement-style axis the shared model always specified but the code never selected: `MovementStyleService.gd` (new) scores the ten styles deterministically per purpose, and `BehaviorArbiter`'s `DecisionTrace` now carries `movement_style`. Absorbs all four items V2-COMBAT-003 filed forward: `encounter_id` now names one encounter rather than one stage, so two fights in the same stage get different boards and spawn cells; `data.combat.board` range raised 12..22 → 18..28 (measured: 18x18 gives Courage 81 islands per 50 boards, up from 0 at 12x12); the `resist_fear` trait, which had a working reduction gate but none of its 14 real call sites ever passed the arguments that gate needs, is now wired through combat and 10 previously-inert sites — it did nothing anywhere in the game before this story, not just in combat; and enemy-directive faction gating, since the player's active combat directive was biasing enemy AI scoring too, now gated to `faction == "echo"`. Phase 6's own combined verification then found the movement-style axis's headline claim didn't fully hold in live play — measured 40 real fights, and while `retreating`'s absence was expected (it needs a real collapse state, none occurred in the sample), `lateral` and `low_exposure` were real bugs: `lateral` had no live trigger (only ever built for goal types live combat never creates), fixed by giving it a genuine flanking-cutoff trigger toward a perceived hostile, matching movement-model.md section 9's own "lateral cut-off" reading; `low_exposure` silently collapsed into `safe` on every hazard-free board (both only weighed exposure and hazard count), fixed by adding hostile-distance as a tiebreak with hazard safety kept as the strict first priority. Phase 7's own full-regression pass then found a real, pre-existing bug unrelated to this story's scope: in GUIDE_SPIRIT escort fights the spirit is built with `is_structure: true`, and `MovementProfileService.derive_profile()` checked that flag before any capacity override, so the spirit always got capacity 0 and could never move — broken since a July commit (V2-COMBAT-002 Slice 3), not caused by this story, and consequential because about half of all GUIDE_SPIRIT fights are escort mode. Fixed by reordering the check; verified live, escort wins via `spirit_escorted` went from 0/11 to 2/11 in an 11-fight sample. A second bug surfaced once capacity was restored: a guarding Echo could block the spirit's path indefinitely (measured: one fight blocked 20 rounds straight). Because this changes party AI behavior rather than being a pure code fix, it went to `sr-game-designer` for a design pass, which recommended a deterministic yield/swap — every round, if the spirit's planned next cell holds a living, friendly, non-structure Echo, the two trade positions as part of the spirit's own activation, at no cost to the Echo, and never for a hostile, dead, downed, structure, or spirit occupant. `game-feel-developer` added a readability requirement, since a repeating unmarked swap reads as a stuck loop rather than the party protecting the spirit, closed with one new `spirit_escort_yield` bark. `qa-verifier`'s independent review of the combined fix returned SHIP and found one adjacent gap: none of the 5 `spirit_*` bark contexts were listed in `data.voice.bark_tiers`, so they defaulted to lowest priority and could be silently dropped in a busy round, including the new yield bark that was explicitly required not to ship silent — fixed by adding all 5 to tier 2. The PR then went through `/ultrareview`, which found 2 real nit-severity findings in the swap mechanic (a wasted recursive deep-copy, a duplicate eligibility lookup computed twice per round) — both fixed and independently re-verified. Jeff's own Phase 8 in-game playtest confirmed both GUIDE_SPIRIT modes work correctly live (bark fires, `GUIDE_SPIRIT escort progress` logs every round, clean victories in both protect and escort mode), and surfaced two more pre-existing, unrelated issues, both filed as follow-ups rather than fixed in this story: GUIDE_SPIRIT/PURSUE boards on a low-plateau-count virtue (Courage, realm.01 "Ashen Hallow") look compact instead of stretched, because `StageTerrain.generate()`'s plateau count/size comes only from the per-virtue terrain signature with no scaling relative to the board-stretch override; and the combat camera does not handle a genuinely large stretched board well. Fifteen follow-up tasks in total were filed to `docs/stories/v2-combat-003.5/followup-tasks.md`, each with a full opening prompt; the complete 64-entry decision trail lives in `docs/stories/v2-combat-003.5/decisions.md`.
