...
- **Region guard (V2-COMBAT-003 phase 2c/2c-region):** passes 1–2 restrict BOTH factions to the board's single largest connected region — computed once per `place_actors` call via `StageTerrain.legal_neighbors` flood fill, ties broken by the numerically-lowest `col,row` cell — so an actor is never placed in a cut-off pocket the party (or the enemies) cannot reach, and the two factions can never end up split across separate regions. Pass 3 is an unfiltered last resort over the full walkable set so no actor is ever dropped. `place_actors` returns `{echo,enemy}_outside_region_fallback` (that faction's share of the main region ran out, though the walkable set had room elsewhere) and `{echo,enemy}_walkable_exhausted_fallback` (more actors than walkable cells, board-wide — a plain shortage, not a connectivity defect) — kept distinct because collapsing them into one flag was a measured defect.
...
### Shared Movement Contract, Pressure + Route-Arbitration Foundation (V2-COMBAT-002 Slices 1–5)
...
**Slice 3 — profile derivation, shared executor, fixed hazards, ordered activation (dormant).** Five new pure/static services under `core/movement/`, all deterministic, none wired into live combat (live cutover is Slice 6):

- **`MovementProfileService.derive_profile(actor, capacity_cfg, options={})`** — the single dormant source of live movement capacity. Capacity formula is **FROZEN**: `final = clamp(max(standing_capacity, aptitude_capacity), 2, 6)` (a `max()`, never a sum). `standing_capacity` bands S1–2 → 2, S3–5 → 3, S6–9 and above → 4 (ceiling 4; Standing 9 gets **no** auto-increase). `aptitude_capacity = 2 + (agi≥12:+1) + (agi≥18:+1) + Calling kra_soro (+1) + skill kra_soro_open_ground (+1)`; there is **no Standing term in aptitude** and `sum_okwanfo_shadow_step` is deferred. Reads Standing from `actor["standing"]` (falls back to `rank`), agi from `actor["stats"]["agi"]`, Calling from `actor["calling"]`, skills from `actor["equipped_skills"]`. Structures → capacity 0. A non-joining GUIDE spirit gets a capacity-1 authored override **only via an explicit `options.authored_override`** (never inferred from `is_spirit`). No `max_movement` is persisted to save; equipment-driven capacity is deferred to V2-ITEM-003.
- **`MovementHazardService`** (+ fixed fixtures in `MovementHazardFixtures.gd`) — three fixed hazard types (`unstable`, `binding`, `burning`) with `resolve_cell_entry` and `resolve_end_activation`. A once-per-type-per-activation ledger (`{triggered:{unstable,binding,burning}}`) guarantees each hazard type fires at most once per activation. Unstable displacement is ranked by outward-progress, then angular deviation, with **no row/col bias**; a genuine tie yields `fallback_damage`. Forced displacement is free and cannot recursively retrigger Unstable.
- **`MovementExecutor.execute(context, intent, profile, hazard_ctx)`** — pure edge-by-edge traversal. Two-solid-corners diagonal legality goes through `StageTerrain.is_legal_edge` (occupancy is never terrain solidity — same rule as Slices 1–2). Dynamic occupancy is honored per edge; a hostile-controlled edge adds a once-per-edge `+1` surcharge (control sources sorted). Forced displacement is free. Events are emitted in one strictly ordered sequence; stop reasons are `reached_destination`, `commitment_spent`, `capacity_spent`, `no_route`, `blocked_edge`, `occupied`, `binding_stop`.
- **`CombatActivationService.activate(context, intent, profile, hazard_ctx, action_ctx={})`** — the ordered movement-plus-action pipeline: executor → action revalidation at the final cell → purpose-restricted fallback → declared resolved action (**declared, not executed**) → Burning end-of-activation resolution → KO/death truth → exactly one validated `MovementResult`.
- **Additive edits:** `HAZARD_TYPES` const added to `MovementKnownHazardFact.gd`; `data.combat.movement.{capacity, hazards}` added to `balance.json` (the `3/3` hazard damages are **PROPOSED DEFAULTS** pending Jeff's Slice-6 tuning); Slice-3 test suites registered in `AppRoot`.

**Slice 3 acceptance:** 1149/1149 full-suite tests (baseline 1083 → 1149), 0 failed; compile clean; five independent reviews all SHIP (0 blockers, 0 majors). **Dormant only** — nothing is wired into live combat; the live cutover is Slice 6. **Slice-6 carry-forward guardrails surfaced by review (NOT Slice-3 defects):** reconcile the `controlling_state` default between `MovementExecutor` (false) and `MovementProfileService` (true); guard `hazard_ctx.config` presence at the live call site; drive `mover_ko_only` from live actor state; ratify `_PURPOSE_FALLBACK_ALLOW`; ensure the live caller declares the guard/observe fallback; and confirm the `3/3` hazard damage against tuning. Special carry/retreat/interpose/protect/pursue mechanics → Slice 4, party stage-movement adapter → Slice 5, playable presentation + live cutover → Slice 6.

**Slice 4 — escape authority, guide-spirit activation, protect custody, board fall-back (dormant).** Three new pure/static services under `core/movement/` plus two additive extensions, all deterministic, none wired into live combat (live cutover is still Slice 6):

- **`PursueEscapeService`** — the **single escape authority** for PURSUE. Exposes `escape_cells`, `is_escaped`, `escape_graph`, and `cutoff_cells`. The `>= max - 1` **band** is the one escape predicate (`_is_escape_geometry`) along the board's long axis; `is_escaped` byte-replicates the live rule at `FlowRuntime.gd:2082-2087` so the Slice-6 cutover is **behavior-preserving**. `cutoff_cells` projects a shortest-route corridor off the escape graph — cells where `cost_to_quarry + cost_to_escape == min_escape_cost` — filtered to those a pursuer can reach no later than the quarry. This consolidates **four previously inconsistent inline escape computations** into one seam. **PURSUE quarry timing and economy are preserved unchanged; only cutoff *targeting* moves.**
- **`GuideSpiritActivationService.activate_spirit(spirit_actor, context, guide_state, hazard_ctx, capacity_cfg)`** — the non-joining spirit's objective-phase movement expressed as an explicit NPC activation through `CombatActivationService` plus hazards, rather than bespoke board movement. Authored one-cell pace comes from an `authored_override` capacity of 1. A hard `joined` precondition **refuses** joined spirits (they remain arbiter-driven). Escort takes the first step of `shortest_path`; skittish picks a neighbour ranked Chebyshev → Manhattan → threat-distance vector → obstruction-distance vector → numeric, which is **reflection-covariant** (no board-orientation bias). The spirit scores **no** objective progress (forced `0.0`) — objective truth stays with the existing round-end authority.
- **`ProtectCustodyService`** — PROTECT custody mechanics over an **immutable** `custody_state`. **Pickup IS the activation's single primary action** (`protect.totem_pickup`, `pickup_range` 1) — it is never a free side effect of moving. The totem follows its carrier across **both** voluntary and forced steps. Carrying costs **−1 carrier capacity**, applied so that the capacity-1-requires-`authored_override` invariant from Slice 1 still holds. **Theft transfers only on a successful ATTACK** (roll injected by the caller, strict `<`, **no internal RNG**) and **never on cell entry** — the no-op on entry is explicitly tested. An enemy carrier is move- and action-restricted and takes **double damage** via the existing `double_damage_mult` (no new multiplier invented). Drop/recovery cell is the carrier's current cell.
- **`CombatPressureService` extensions** — board fall-back reuses the **existing `withdraw` purpose**; no new purpose or option style was invented and `RetreatService` is untouched (see the design decisions below). For PURSUE the escape-graph cutoff is **PRIMARY**, overriding the authored `fallback_region`. For PROTECT, a stolen-totem cutoff projects from the **carrier's own** escape graph. Interposition/screen regions come from a Chebyshev-geodesic predicate, bounds- and walkability-filtered.
- **`MovementExecutor` extension (reaches into Slice-3 code).** Authored-override movers now get a **one-STEP pace that is independent of edge cost**: `step_allowance = mini(authored_override.capacity, commitment)` counted in **EDGES**, so a single edge is always affordable regardless of terrain cost or the `+1` hostile-control surcharge. Normal movers keep **byte-identical** cost-budget semantics. `voluntary_cost` still records the true spend and `remaining_capacity` clamps at 0.

**Slice 4 approved design decisions (Jeff):**
1. Custody model as above, including pickup-as-primary-action and theft-only-on-attack.
2. Board retreat reuses the existing `withdraw` purpose. "Withdraw from combat entirely" remains the separate `RetreatService` and is **out of scope**.
3. PURSUE quarry timing/economy is **preserved**; only cutoff targeting changed.
4. PURSUE cutoff is an escape-graph projection and is **primary over authored terrain**.
5. **Authored one-cell pace is a STEP COUNT, not a cost budget** (`docs/movement-model.md` §13.7). This pins a **confirmed bug**: under a cost budget the non-joining spirit could never move when a hostile was adjacent or terrain cost was ≥ 2 — PROTECT mode was inert exactly when it mattered, and it was a regression against V1's cost-free BFS.
6. That pace rule keys off the `authored_override` **contract**, so it also covers burdened PROTECT carriers (including enemy carriers capped to 1). Jeff explicitly confirmed this broader application.
7. **Action economy is UNCHANGED** — capacity governs movement only, and one primary action follows movement. A unified action-point pool was proposed and **deferred to its own story** (see `docs/proposals/action-point-economy.md`, untracked planning area).

**Slice 4 acceptance:** **1233/1233 full-suite tests** (baseline 1149 → 1233, **+84**), 0 failed; compile clean. Five independent reviews (architecture, determinism, movement-design, mode-correctness, test-coverage) plus a **targeted second pass** over the surface the first pass never reached (`GuideSpiritActivationService` and the four new test suites). Three genuine defects were caught and fixed: (a) a malformed ternary that made `escape_cells` return the far-end line instead of the band, (b) the authored-pace step-count pin above, (c) two un-reverted mutation-test edits left in the tree. Jeff's manual in-game regression pass confirmed nothing regressed. **Dormant only** — nothing is wired into live combat; the live cutover is Slice 6. Slice-6 carry-forward items surfaced by review (**NOT Slice-4 defects**) are recorded in `docs/integration-map.md`. Party stage-movement adapter → Slice 5; playable presentation + live cutover → Slice 6.

**Slice 5 — party stage adapter (dormant) + the `traveled_path` convention correction (LIVE).** One new pure/static file plus one authorized live correction:

- **`core/movement/StagePartyMovementAdapter.gd` (new, dormant)** — converts party/directive state into the slice-1..4 contract shapes for STAGE traversal: `build_profile` / `build_goal` / `build_intent` / `build_result` / `select_frontier` / `select_objective_target`. Static functions only, plain Dictionaries in and contract Dictionaries out, no RNG, no `OS` time, zero references to `FlowRuntime` / `FlowContext` / `SaveService` / any UI node, and **zero production callers**. **Stage traversal is ONE PARTY TOKEN** (`MOVER_ID = "party.token"`): the API deliberately exposes no per-Echo seam — no `mover_id` parameter, no actor array, no per-actor capacity. Adding one would be a design change, not a refactor. Capacity is the **directive `step_budget`** (Scout Carefully 3 / Seek Signs 6), not the clamp(2,6) COMBAT capacity formula; a budget below 2 yields capacity 1 plus an `authored_override` naming `directive.step_budget` rather than silently clamping up. `controlling_state` is false — a party token projects no hostile control. Reachability routes through `MovementPathService`, so the shared two-solid-corners diagonal rule applies; `StageTerrain.bfs_distance_field` is **deliberately not used** because it expands neighbours with a raw `walkable.has()` test and therefore ignores that rule. Every returned dict passes the corresponding slice-1..4 `validate()`.
- **`MovementGoal.MODES` gained `"explore"`** — an allowlist append, so it cannot invalidate a goal that validated before the entry existed. It replaces the false `recover` segment in stage `goal_id`s (thread recovery is the run's reward, not the movement mode).
- **`traveled_path` convention correction (LIVE, Jeff-approved).** Stage traversal now obeys the movement contracts' path-excludes-origin rule — see § Stage Exploration above for the full snapshot/save/UI contract. Save change is **additive**: `explore_map.last_traveled_origin` is new with a `{}` default and a `SaveService` repair alongside the existing `last_traveled_path` backfill; `last_traveled_path` is neither removed nor renamed. On-screen behaviour is unchanged.

**Slice 5 acceptance:** **1256/1256 full-suite tests** (baseline 1233 → 1256, **+23**), 0 failed. **Everything dormant except the path-convention correction**, which is the one live behaviour change in the slice and is presentation-only. Live exploration ROUTING is untouched: `StageTerrain.nearest_unexplored`, `StageTerrain.next_step`, and `FlowRuntime._find_explore_target` tiers 1–2 were **not** modified — the corrected edge/tie-break rules exist only inside the dormant adapter and go live at the Slice-6 cutover. **Known bias, pinned not fixed:** the frontier tie-break's final determinism guard is reachable and currently governs selection on symmetric boards, because no `heading` value is produced anywhere in `core/` today. A characterisation test pins the current biased answers; it is scheduled for **removal, not repair**, at Slice 6. Slice-6 carry-forward items surfaced by this slice (**NOT Slice-5 defects**) are recorded in `docs/integration-map.md`, led by the `heading` risk. Playable presentation + live cutover → Slice 6.
...
**Terrain (`core/realms/StageTerrain.gd`, pure-static):** stages are irregular landscapes — 2–4 organic plateaus (seeded border-erosion blobs, `plateau.cells`), ≥2-wide connectivity bridges, **islands**; everything else is **uncrossable void**. **Connectivity is judged by a full SHARED SIDE** (V2-COMBAT-003 terrain commit 2) — two cells are in one region only when they share a side, never at a corner. This is deliberately stricter than `is_legal_edge`, which permits a diagonal unless BOTH orthogonal sides are solid; strict is the safe direction, because everything the generator calls connected is certainly traversable in play. The repair bridges every cut-off region of **`connect_min_region_cells`** (default 6) cells or more into the **host region** (largest, ties by numerically lowest col,row — the same host rule `GridService._largest_walkable_region` uses to place actors). Smaller regions are **left alone on purpose**: small islands are terrain variety. Before this change the region test was plain 8-direction, so two plateaus touching at one corner counted as connected and the repair never fired — measured cut-off regions reached 176 cells. Generated deterministically from a **per-realm landscape signature**. `walkable_set(terrain)` (empty terrain ⇒ legacy full-rect), `bfs_distance_field`/`next_step` (walkable-aware pathing — the party never enters void or dead-ends), `cells_within_radius`, `nearest_unexplored`. New append-only RNG: `stage.N.explore.terrain.{bounds,plateau.K,plateau.K.shape,bridge.K,island.count,island.K,island.K.bridge}`, `…sit.N.wpos.K`. **V2-COMBAT-003 terrain commit 3 RENAMED `straggler.count`/`straggler.K` to `island.count`/`island.K`** and an island now makes MORE than one draw on its stream (size, seed cell, one per growth step) where a straggler made exactly one — each island still derives its OWN stream, so one island's draws can never shift another's. **No namespace was added, renamed or reordered by the shared-side change** — but the repair now fires where it used to be blind, so a board consumes MORE `bridge.K` streams than before and the density-driven extra bridges continue from a higher K. Every board in the project therefore differs from its pre-commit-2 shape; the fingerprints were re-recorded once, deliberately.

**Islands (`StageTerrain`, V2-COMBAT-003 terrain commit 3 — replaces "stragglers"):** an island is a **deliberate design feature**, not a defect (handoff decision 14). It is minted **after** the connectivity repair, so the repair's "exactly one region of ≥ `connect_min_region_cells` cells" guarantee covers **plateaus plus bridges, never the whole walkable set** — a test that asserts it against `walkable_set` is testing the island pass by mistake. Three properties hold **by construction**, each asserted by a test: (1) **moated** — no island cell is adjacent in **any of the 8 directions** to a cell of any other region, island or mainland, because an island may only occupy cells outside the 8-direction dilation of everything walkable so far; a corner touch is impossible, not merely rare; (2) **multi-cell and shared-side contiguous** — growth only adds cells sharing a full side, minimum **4** cells, and a blob that cannot reach 4 is discarded rather than emitted; (3) **sized to the board** — authored `island_size_min/max` is a **request**, clamped to at most `area/16` per island and `area/4` for all islands together (floor 4), so `wisdom`'s authored 8–50 gives ≤ 9 cells per island on a 12×12 combat board and the full range on an explore map. Growth prefers the frontier cell with the most 8-direction contacts, so islands are blobs and not one-cell-wide worms. **Before the change, 819 of 865 cut-off regions were a single cell and 838 of 865 touched the main ground at a corner** — a straggler was one cell taken from the 8-direction neighbours of existing ground, so it could never be an island and realms differentiated only by *how many* single-cell bumps they got. **`StageTerrain.walkable_set` reads BOTH `islands` and the legacy `stragglers` key**: terrain is persisted (`FlowStageExploreState` calls it permanent geometry), and dropping the old key would make an already-saved board silently lose ground with no error and no failing test. **`entry_cell` now anchors to the host region** (largest shared-side region, ties by lowest col,row) — the leftmost walkable column can be an island, and a moated island is cut off under the explore layer's 8-direction fill too, so the old rule would have started the party on ground with no route off it. **Host-region placement landed in terrain commit 5** (below), and **island bridging in terrain commit 4** (below) — some islands are now reachable, and the moat survives for every island that is not.

**Host-region placement and the objective site (`GridService`, `RealmGenerator`, `StageTerrain`, V2-COMBAT-003 terrain commit 5 — decisions 22, 24, 25):** **nothing spawns outside the host region** — no Echo, no enemy, no objective, no situation. Islands are pure scenery. There is **exactly one host-region authority**, `GridService.largest_walkable_region(walkable, bounds)` (largest region under `StageTerrain.legal_neighbors`, ties by numerically lowest `col,row`); `RealmGenerator._place_situations` calls that same function, so the explore layer and the combat layer can never disagree about which region is the host. Measured on 1,800 boards per regime, ten virtue signatures: a shared-side host cell outside the legal-edge host on **0 of 1,800** boards, both regimes, so `StageTerrain.entry_cell`'s shared-side host is always inside it. `GridService.collect_unoccupied_cells(walkable, occupied, region_set)` takes the region as an **optional third argument** — every one of the six post-placement spawns (shrine, RECOVER relic, PROTECT entity, PURSUE quarry, GUIDE_SPIRIT spirit and its escort destination, the temporary ally) passes it; omitting it restores the pre-commit-5 unfiltered behaviour byte-for-byte and exists only for a caller with no bounds to hand. **A static objective needs all EIGHT neighbouring tiles walkable and free** — a MINIMUM, not a target. `GridService.place_on_terrain(..., clearance_ctx)` adds two ranking keys when given `{ "walkable", "occupied" }`: clearance first, ahead of everything, and `openness` (walkable free cells in the 5×5 neighbourhood) as a tie-break **inside** the depth metric, so the depth intent still chooses among the cells that clear the minimum. The five objective spawns pass it; the ally does not — it is a combatant that wants the cell nearest the party. On the explore side an objective situation must clear its eight neighbours in every placement tier, the sorted last-resort pool included. **If no region can host an objective the generator builds one** (`StageTerrain._find_objective_site`): a 3×3 block, one centre with eight around it, joined to the host region, chosen with **no RNG** by most overlap with existing ground then lowest centre, and run **after** the connectivity repair and **before** the island pass so the moat is computed against it. It is recorded as `terrain["objective_site_built"]`, a key present **only when the pass fired**, so a board that never needed it is byte-identical to one generated before the pass existed. Measured on 1,800 boards per regime: it fired **0 times**, on every one of the ten virtues — every generated board already offers a legal objective site, and the compensation is a guarantee rather than a repair in daily use. Before commit 5, measured the same way: **1,146 of 9,000 situations (combat bounds) and 1,110 of 9,000 (explore bounds) landed off the host region**, 392 and 408 of them objectives, 1,629 and 2,059 objectives without clearance, and 33 and 137 of 1,800 combat objective spawns off the host region. All ten figures are **0** after.

**Island bridges and the bridge tile (`StageTerrain`, `CombatBoardScreen`, `StageExploreScreen`, V2-COMBAT-003 terrain commit 4 — decisions 15-19):** **A bridge is its own tile.** `StageTerrain.bridge_cell_set(terrain)` returns the union of the `bridges` rects as a `"col,row"` set, always a subset of `walkable_set`; both boards paint an ordinary ground tile and then a tinted **`BridgeLayer`** overlay on top of it. That layer is a **child of `Board`** in both scenes, so it inherits every pan, zoom and centring transform with no sync code, and the board silhouette and isometric overlap order are unchanged. The tint is a **PLACEHOLDER**; the art keys off the same `relief` seam. This surfaces a distinction the data already carried — `plateaus`, `bridges` and `islands` have always been separate lists and only `walkable_set` flattened them — and adds no terrain concept. **An island of 6 cells or more may be bridged**, on the per-realm **`island_bridge_chance`** (PROPOSED DEFAULT on all eleven `map_shape` entries: acceptance 0.0, truth 0.2, courage 0.3, humility 0.4, empathy 0.5, compassion 0.5, generosity 0.6, wisdom 0.6, leadership 0.7, forgiveness 0.8, default 0.3). An island of **5 cells or fewer is never bridged** — it stays pure scenery. **An island of 20 cells or more may take extra bridges**, at most **one per side**, each reaching a **different** neighbouring region, which may be another island; each additional side is offered at the same per-realm chance. Below 20 an island takes at most one bridge. **A bridge lands anywhere along an edge.** Every legal departure position on every legal side is enumerated first and **one** draw picks among them, so two islands of the same size do not produce the same picture; the ray stops one cell short of the first ground it meets, spans at least 2 cells (a 1-cell span would be a `1 x bridge_width` rect and would break `terrain/bridge_width_min2`), and joins exactly **one** region. **The unbridged moat survives by construction, not by inspection:** a candidate bridge cell is rejected when any cell of its own 3x3 neighbourhood belongs to an island other than the source and the target, so no bridge is ever adjacent — side or corner — to a third island, whatever that island's own outcome turns out to be. Ordering therefore cannot leak. **A bridged island joins the host region**, so it becomes legitimate ground to spawn on and a legal objective site; both rules are evaluated downstream on the finished walkable set. An island bridge is an ordinary bridge rect carrying three **optional** keys — `island_bridge: true`, `island_index`, `target_island` (-1 = the mainland). Nothing in the walkable path reads them; **a test that asserts terrain commit 2's guarantee against `bridges` must strip them**, because an island-to-island bridge is part of the island system and would otherwise read as a cut-off bridge region. **THE EROSION LEFTOVERS (decision 15).** Plateau erosion used to test **8-connectivity** of the candidate blob, which accepts a cell joined to the rest of the plateau by a single **diagonal**. Under the shared-side rule that cell is its own region of ONE, so the connectivity repair — which only acts on regions of `connect_min_region_cells` or more — left it alone and it shipped as ground touching a plateau **at a corner only**: measured **426 across 1,600 boards**, attributed island=0 **plateau=426** bridge=0. Erosion now tests **shared-side** connectivity, which holds the invariant at every erosion step, so a plateau blob is exactly one shared-side region and can no longer shed a corner-hung cell — the erosion that would have orphaned it is refused, and the cell is absorbed rather than removed. Board geometry moved once, on purpose. RNG: **`{prefix}.island.{k}.bridge`**, one stream per island and **separate** from `{prefix}.island.{k}`, so a bridge draw can never shift an island's shape. An island below 6 cells makes no draw there at all.

**Per-realm landscape signatures (`balance.json → data.stages.map_shape`):** `default` + `by_virtue` with all **10** virtue-keyed signatures (the 10 Realms map 1:1 to the 10 virtue domains; only realm.01 Courage + realm.02 Wisdom exist in `realms.json` today — the rest inherit by virtue). `RealmGenerator._resolve_terrain_signature(realm_cfg, stages_cfg)` order: realm-entry `terrain_signature` override → `by_virtue[virtue]` → `default`. Each signature carries plateau count/size/shape-bias, bridge width/density, **`island_count_min/max` + `island_size_min/max` + `island_bridge_chance`** (V2-COMBAT-003 terrain commits 3 and 4 — replaces `straggler_count_min/max`; all eleven entries are **PROPOSED DEFAULTS** pending Jeff's play tuning, see `docs/stories/v2-combat-003/handoff.md` §12.5), `connect_min_region_cells` (the smallest cut-off region the connectivity repair bridges back in; floor 2, default 6), and a **`relief` descriptor string RESERVED for future per-realm visual assets** — **art will key off realm.id / virtue + `relief`; do not lose or rename this seam.** Stages within a realm vary deterministically off the stage seed (no two identical) yet share the realm's character.
...
**Durable run-state:** `explored_cells`, `revealed`, `resolved`, `passed` persist across `return_home` → re-entry (and a different party) — `FlowStageExploreState._reset_session_state` carries them; only transients (`pending_situation_id`, `in_transit`, `target_situation_id`, `last_traveled_path`, `last_traveled_origin`) clear. Reset on stage regeneration (realm replay).

**Snapshot (`flow.stage_explore` + overview `flow.stage`):** `data.terrain`, `data.explored_cells`, `data.situations` (only **revealed** entries, each with `passed`), `step_budget`, `steps_to_target`, `in_transit`, `target_situation_id`, `traveled_path`, `traveled_origin`. The overview emits no situation markers + the discovered-fog map + objective briefing (objective positions never exposed pre-entry).

**Path excludes origin (corrected V2-COMBAT-002 slice 5 — LIVE):** a traveled path holds **destinations only**, one entry per cell actually entered; the cell departed FROM is never an element of it. This is the same rule the movement contracts enforce (`MovementContractValidation.require_path_excludes_origin`), and stage traversal now obeys it: `data.traveled_path` / `explore_map.last_traveled_path` are destinations-only, and the pre-advance cell rides alongside in the additive `data.traveled_origin` / `explore_map.last_traveled_origin`. A zero-step advance yields an EMPTY path (never `[origin]`) with `traveled_origin` = the unchanged party cell. `_project_traveled_origin` falls back to `party_pos` when the field is missing or `{}` (session reset, repaired legacy save), so the UI always has a tween anchor. The UI re-anchors segment 0 at `traveled_origin` and takes `seg_count = traveled_path.size()`; **rendered animation, ghost trail and step-diamond count are unchanged.**

**Traversed path in the encounter snapshot (V2-COMBAT-002 slice 6D — LIVE):** `snapshot.data.last_actor_action` carries two additive keys, `from_pos {col,row}` (the mover's PRE-activation cell) and `path` (Array of `{col,row}`). `path` is `MovementResult.actual_traversed_cells` **verbatim** — destinations only, origin excluded, chronological, interleaving voluntary steps and forced hazard displacements, and it MAY be empty. It is never reconstructed or normalised, so a contract violation surfaces rather than being papered over. Both keys are ALWAYS present so the shape is stable, and they are stamped for EVERY `action_type` (a move-then-melee activation reports the melee entry, which must still carry the walk). Presentation-only: no simulation, RNG, or save behaviour reads it. `snapshot.data`'s own key count is unchanged.

**Save (additive, `if not has`):** `explore_map.terrain {}`, `explored_cells {}`, `in_transit false`, `target_situation_id ""`, `last_traveled_path []`, `last_traveled_origin {}` (V2-COMBAT-002 slice 5 — **additive**; `last_traveled_path` is neither removed nor renamed); per-situation `passed` defaults false via `.get`.
