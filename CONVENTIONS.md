...
| CombatBoardScreen | `flow.encounter` | actors (projected, incl. `is_quarry`), round, round_phase, initiative_order, objective_state, retreat fields (pre_combat only) | nav.back, cta.retreat (when eligible). **PURSUE:** auto-follow camera via manual board repositioning — `CombatBoardScreen` extends `Control` (no Camera2D node). `_apply_board_transform(pos: Vector2)` repositions all five layers simultaneously: `_board`, `_token_layer`, `_move_telegraph_layer`, `_distance_layer`, `_bark_popup_layer`. Quarry tracked via `is_quarry` flag in token dict; `_pursue_mode: bool` state var gates the auto-follow `_process` lerp. Player can pan (2-finger drag) or zoom (pinch) to override; auto-follow resumes after `_PAN_RESUME_DELAY` (3 s). Quarry gold-diamond badge overlay in `CombatTokenLayer`. |
