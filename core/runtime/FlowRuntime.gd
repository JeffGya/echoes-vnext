	...
	match action_type:
		...
		"flow.go_state":
			...
			if to_state == FlowStateIds.SANCTUM:
				...
				# V2-VOW-002: evaluate stage-entry vow condition on STAGE_EXPLORE entry (covers both
				# first entry via flow.select_stage → STAGE → cta.start and re-entry after defeat).
		...
		"flow.select_stage":
			...
			# V2-VOW-002: vow entry condition moved to flow.go_state → STAGE_EXPLORE so it also
			# fires on re-entry paths (defeat → change party → re-enter same stage).
...
func _check_vow_discovery(t: int) -> void:
	...
	for vow_id in defs:
		...
		if VowService.evaluate_discovery_scenario(scenario, party_actors, situations, is_victory):
			...
			# V2-VOW-002: record in session_unlocked_vows for "Vow Revealed" on ResolveScreen
...
# V2-VOW-002: Called on flow.go_state → STAGE_EXPLORE (covers first entry and re-entry after defeat).
func _apply_vow_stage_entry_condition(t: int) -> void:
	...
	# V2-VOW-002: clear stale outcome and break chip on every stage enter.
